...
def test_read():
    ...
    assert tle.epoch == datetime(2008, 9, 21, 12, 25, 40, 104192)
    assert tle.ndot == -2.182e-5
    assert tle.ndotdot == 0.
    assert tle.drag == -0.11606e-4
    assert tle.i == 51.6416
    assert tle.Ω == 247.4627
    assert tle.e == 6.703e-4
    assert tle.ω == 130.5360
    assert tle.M == 325.0288
    assert tle.n == 15.72125391
